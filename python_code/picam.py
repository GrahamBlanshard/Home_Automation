...
# Custom Modification by Graham for personal use. Comments added where applicable prefixed by GB
import StringIO
import subprocess
import os
...
filepath = "/mnt/security/garage"	#GB NAS mount point
filenamePrefix = "garage-cap"		#GB Custom pref, adds potential for more cams later
diskSpaceToReserve = 40 * 1024 * 1024 # Keep 40 mb free on disk
cameraSettings = "-vf"	#Vertical Flips

# settings of the photos to save
saveWidth   = 1280
saveHeight  = 720
saveQuality = 80 # Set jpeg quality (0 to 100)
...
def saveImage(settings, width, height, quality, diskSpaceToReserve):
    ...
    time = datetime.now()
    savepath = filepath + "/%04d%02d%02d/" % (time.year, time.month, time.day) #GB Custom folders for continued use and sorting
    try:	#GB Generate the folder if it doesn't exist
        os.stat(savepath)
    except:
        os.mkdir(savepath)
    filename = savepath + filenamePrefix + "%02d%02d%02d.jpg" % (time.hour, time.minute, time.second)
...
while (True):
    ...
    if takePicture:
        if time.time() - lastCapture >= 1: #Once per second max
            lastCapture = time.time()
